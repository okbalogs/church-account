...
import { ACCOUNT_CATEGORIES } from "../../constants/categories";
...
export default function IncomeHistoryScreen() {
  ...
  const fetchFromApi = useCallback(async () => {
    ...
    try {
      setError(null);
      const data = await apiFetch<CachedEntry[]>("/api/entries");
      await setIncomeCache(data);
      setEntries(data);
    } catch (e) {
  ...
  async function handleDelete(id: number, date: string) {
    Alert.alert("Delete Entry", `Delete entry for ${date}?`, [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete", style: "destructive",
        onPress: async () => {
          const prev = await getIncomeCache();
          await setIncomeCache(prev.filter(e => e.id !== id));
          ...
          try {
            ...
            if (isNetworkError(e)) {
              await enqueue({ opId: String(Date.now()), type: "DELETE", path: `/api/entries/${id}` });
              await refreshPendingCount();
            } else {
              await setIncomeCache(prev);
              setEntries(prev);
  ...
  if (error && entries.length === 0) {
    return (
      <SafeAreaView style={styles.center}>
        ...
        <TouchableOpacity style={[styles.btn, { backgroundColor: C.income }]} onPress={() => { setLoading(true); loadFromCache().then(() => fetchFromApi()); }}>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["bottom"]}>
      ...
      <FlatList
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchFromApi(); }} tintColor={C.income} />}
        ListHeaderComponent={
          <View style={styles.header}>
            {entries.length > 0 && (
              <View style={[styles.totalBanner, { backgroundColor: C.income }]}>
                <Text style={styles.bannerLabel}>Overall Total Income</Text>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <TouchableOpacity style={[styles.btn, { backgroundColor: C.income }]} onPress={() => router.push("/(tabs)/income")}>
        ...
        renderItem={({ item: e }) => {
          ...
          return (
            <TouchableOpacity
              style={[styles.card, e.pending && styles.cardPending]}
              onPress={() => setExpanded(isExpanded ? null : e.id)}
              activeOpacity={0.8}
            >
              <View style={styles.cardTop}>
                <View style={styles.cardMeta}>
                  ...
                  <View style={styles.badge}>
                    <Text style={[styles.badgeText, { color: C.income }]}>{e.service_type}</Text>
                  </View>
                </View>
                <Text style={[styles.cardTotal, { color: C.income }]}>{fc(Number(e.grand_total))}</Text>
              </View>
              ...
              {isExpanded && (
                <>
                  ...
                  <View style={styles.subTotals}>
                    ...
                    <View style={[styles.subTotal, { backgroundColor: C.income }]}>
                  ...
                  <View style={styles.cardActions}>
                    <TouchableOpacity
                      ...
                      onPress={() => router.push(`/edit-income/${e.id}` as never)}
                    >
...
const styles = StyleSheet.create({
  ...
  badge: { alignSelf: "flex-start", backgroundColor: C.incomeLight, borderWidth: 1, borderColor: "#a7f3d0", borderRadius: 20, paddingHorizontal: 10, paddingVertical: 3 },
  badgeText: { fontSize: 12, fontWeight: "600" },
  cardTotal: { fontSize: 17, fontWeight: "800" },
  catWrap: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginTop: 12 },
  catChip: { backgroundColor: C.bg, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4 },
  catText: { fontSize: 12, color: C.sub, fontWeight: "500" },
