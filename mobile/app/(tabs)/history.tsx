...
export default function IncomeHistoryScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const data = await apiFetch<Entry[]>("/api/entries");
      setEntries(data);
    } catch (e) {
  ...
  if (error) {
    return (
      <SafeAreaView style={styles.center}>
        ...
        <TouchableOpacity style={[styles.btn, { backgroundColor: C.income }]} onPress={load}>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["bottom"]}>
      <FlatList
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); }} tintColor={C.income} />}
        ListHeaderComponent={
