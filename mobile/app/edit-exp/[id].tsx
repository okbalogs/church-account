...
export default function EditExpScreen() {
  ...
  useEffect(() => {
    async function loadEntry() {
      ...
      try {
        const data = await apiFetch<FormData>(`/api/expenditure/${id}`);
        setForm(data);
      } catch (e) {
  ...
  async function handleSave() {
    ...
    await setExpCache(cache.map(e => e.id === numId ? { ...updatedEntry, pending: numId < 0 ? true : e.pending } : e));
    ...
    try {
      const result = await apiFetch<CachedEntry>(`/api/expenditure/${id}`, { method: "PUT", body: JSON.stringify(form) });
      const updated = await getExpCache();
      await setExpCache(updated.map(e => e.id === numId ? { ...result, pending: false } : e));
      router.back();
    } catch (e) {
      if (isNetworkError(e)) {
        await enqueue({ opId: String(Date.now()), type: "PUT", path: `/api/expenditure/${id}`, body: form as Record<string, unknown> });
        const updated = await getExpCache();
        await setExpCache(updated.map(e => e.id === numId ? { ...updatedEntry, pending: true } : e));
